...
void MainWindow::saveImage()
{
    ...
    if (img.save(filepath, "PNG")) showMessage(tr("Изображение успешно сохранено в %1").arg(filepath));
    else showMessage(tr("Изображение не сохранено"));
}
...
void MainWindow::encodeText() {
    ...
    for (int i = messageInfoLength - 1; i >= 0; --i) byteArray.push_back((usedBytes >> i*8) & 0xff);
...
void MainWindow::messageChanged() {
    ...
    usedBytes = ui->plainTextEdit->toPlainText().toUtf8().size();
    if (imageCapacity < usedBytes) showMessage(tr("Размер сообщения превышен на: %1 байт").arg(usedBytes - imageCapacity));
    else showMessage(tr("Ещё можно ввести: %1 байт").arg(imageCapacity - usedBytes));
}
